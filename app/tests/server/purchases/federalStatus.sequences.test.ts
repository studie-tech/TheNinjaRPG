...
/**
 * A revocation that has read the receipt's owner, with a transfer moving the receipt
 * before its first write. Whatever it finds afterwards must equal the sequential order:
 * transfer, then expiry.
 */
const raceRevocationWithTransfer = async (expiresAt: Date | null) => {
  const database = await db();
  await insertUsers([{ userId: DESTINATION, username: "federal-destination" }]);
  const transactionId = nanoid();
  await grantStorePurchase(database, {
    userId: USER,
    transactionId,
    ...
    store: "APPLE",
    isSandbox: false,
    purchasedAt: new Date(Date.now() - 2 * MINUTE),
    expiresAt,
    raw: {},
  });
  const revoking = beforeStatements(database, storeEntitlementRevocation, [
    () =>
      transferStorePurchases(database, {
        fromUserIds: [USER],
  ...
  await revokeFederalStatus(revoking, USER, {
    eventId: nanoid(),
    occurredAt: new Date(),
    productId: "tnr_federal_gold",
    store: "APPLE",
  ...
  return await receiptAndOwners(transactionId);
};

/**
 * The receipt leaves during the revocation's first pass and comes back during its second,
 * so an owner the revocation has already stamped holds it again at the end.
 */
const raceRevocationWithReturningTransfer = async () => {
  const database = await db();
  await insertUsers([{ userId: DESTINATION, username: "federal-destination" }]);
  const transactionId = nanoid();
  await grantStorePurchase(database, {
...
const receiptAndOwners = async (transactionId: string) => {
  const database = await db();
...
describeWithDatabase("federal status across real webhook sequences", () => {
  ...
  it("converges when two transfers overlap", async () => {
    const finalDestination = "federal-transfer-final-destination";
    await insertUsers([
    ...
    const transactionId = await buyFederal("GOLD", 2 * MINUTE);
    const olderAt = new Date(Date.now() - MINUTE);
    // Nothing serialises the two: the second runs whole before the first has written its
    // redirect, and the end state must be what running them in order would give.
    let second: Awaited<ReturnType<typeof transferStorePurchases>> | undefined;
    const first = beforeStatements(database, storePurchaseTransfer, [
      async () => {
        second = await transferStorePurchases(database, {
          fromUserIds: [DESTINATION],
          toUserIds: [finalDestination],
          store: "APPLE",
    ...
    await expect(
      transferStorePurchases(first, {
        fromUserIds: [USER],
        toUserIds: [DESTINATION],
        store: "APPLE",
        occurredAt: olderAt,
      }),
    ).resolves.toMatchObject({ destinationUserId: DESTINATION });
    expect(second).toMatchObject({ destinationUserId: finalDestination });
  ...
  it("converges when a grant overlaps a transfer of its destination", async () => {
    ...
    const transactionId = nanoid();
    // The transfer runs whole between the grant's ownership reads and its receipt insert;
    // the end state must be what running them in order would give.
    let transfer: Awaited<ReturnType<typeof transferStorePurchases>> | undefined;
    const granting = beforeStatements(database, storePurchase, [
      async () => {
        transfer = await transferStorePurchases(database, {
    ...
    expect(transfer).toMatchObject({ destinationUserId: finalUserId });
  ...
  it("retires a receipt that a transfer moved while the revocation was running", async () => {
    const outcome = await raceRevocationWithTransfer(new Date(Date.now() + 30 * DAY));
    expect(outcome.receipt).toMatchObject({ userId: DESTINATION });
    expect(outcome.receipt?.revokedAt).not.toBeNull();
    expect(outcome.source?.federalStatus).toBe("NONE");
    expect(outcome.destination?.federalStatus).toBe("NONE");
  });

  it("retires a moved receipt with no paid-through date as well", async () => {
    const outcome = await raceRevocationWithTransfer(null);
    expect(outcome.receipt).toMatchObject({ userId: DESTINATION });
    expect(outcome.receipt?.revokedAt).not.toBeNull();
    expect(outcome.destination?.federalStatus).toBe("NONE");
  });

  it("retires a receipt that left and came back while the revocation was running", async () => {
    const outcome = await raceRevocationWithReturningTransfer();
    expect(outcome.receipt).toMatchObject({ userId: USER });
    expect(outcome.receipt?.revokedAt).not.toBeNull();
    expect(outcome.source?.federalStatus).toBe("NONE");
    expect(outcome.destination?.federalStatus).toBe("NONE");
  });

  it("lets a CAS loser observe the winning receipt owner", async () => {
    ...
    // The loser has read the stale owner; the winner re-homes the receipt before the
    // loser's own write, whose predicate then matches nothing and sends it round again.
    let winner: Awaited<ReturnType<typeof reconcileStorePurchaseOwner>> | undefined;
    const losing = beforeStatements(database, storePurchase, [
      async () => {
        winner = await reconcileStorePurchaseOwner(database, receipt.id);
      },
    ]);
    const loser = await reconcileStorePurchaseOwner(losing, receipt.id);
    expect(winner).toMatchObject({ userId: destinationUserId });
    expect(loser).toMatchObject({ userId: destinationUserId });
  });
  ...
  it("repairs federal status when expiration retry finds the receipt revoked", async () => {
    ...
    // The receipt is stamped, then the status write fails.
    await expect(
      revokeFederalStatus(failStatements(database, userData), USER, scope),
    ).rejects.toThrow();
    const receipt = await database.query.storePurchase.findFirst({
      columns: { revokedAt: true },
      where: eq(storePurchase.transactionId, transactionId),
  ...
  it("leaves a failed grant pending, then applies it exactly once on retry", async () => {
    ...
    // The receipt insert lands; the balance write that follows fails.
    await expect(
  ...
  it("credits one of two concurrent deliveries exactly once", async () => {
    ...
    // The second delivery runs whole while the first is between its checks and its
    // receipt insert; exactly one of them may credit the balance.
    let second: Awaited<ReturnType<typeof grantStorePurchase>> | undefined;
    const first = beforeStatements(database, storePurchase, [
      async () => {
        second = await grantStorePurchase(database, grant);
      },
    ]);
    const outcomes = [await grantStorePurchase(first, grant), second];
  ...
  it("does not grant a pending subscription after it has expired", async () => {
    ...
    // Leave the receipt pending by failing the atomic status/claim statement.
    await expect(
      grantStorePurchase(failStatements(database, userData), grant),
    ).rejects.toThrow();
