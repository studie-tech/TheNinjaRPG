...
const LEGACY_OFFENCE_KEYS = [
  "ninjutsuOffence",
  "genjutsuOffence",
  "taijutsuOffence",
  "bukijutsuOffence",
] as const;
const LEGACY_DEFENCE_KEYS = [
  "ninjutsuDefence",
  "genjutsuDefence",
  "taijutsuDefence",
  "bukijutsuDefence",
] as const;
const LEGACY_OFFENCE_TO_MASTERY = {
  ninjutsuOffence: "ninjutsuMastery",
  genjutsuOffence: "genjutsuMastery",
  taijutsuOffence: "taijutsuMastery",
  bukijutsuOffence: "bukijutsuMastery",
} as const;

/** Backfill unified combat stats on battles persisted before the offence/mastery split. */
export const normalizeBattleUserCombatStats = (user: BattleUserState) => {
  ...
  for (const [legacyKey, masteryKey] of Object.entries(LEGACY_OFFENCE_TO_MASTERY)) {
    if (legacy[masteryKey] == null) {
      user[masteryKey] = legacy[legacyKey] ?? 10;
    }
  }
  if (user.bloodlineMastery == null) user.bloodlineMastery = 10;
  if (user.sageMastery == null) user.sageMastery = 10;
  user.highestOffence = "offence";
  user.highestDefence = "defence";
  if (!user.highestMasteryType) {
    const masteries = {
  ...
  const used = user.usedStats as Record<string, number> | undefined;
  if (!used) {
    user.usedStats = { offence: 0, defence: 0 };
  } else if (used.offence == null || used.defence == null) {
    user.usedStats = {
      offence:
        (used.offence ?? 0) +
        LEGACY_OFFENCE_KEYS.reduce((sum, key) => sum + (used[key] ?? 0), 0),
      defence:
        (used.defence ?? 0) +
        LEGACY_DEFENCE_KEYS.reduce((sum, key) => sum + (used[key] ?? 0), 0),
    };
  }
};
