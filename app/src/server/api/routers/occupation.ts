  ...
  finishCraftingImmediately: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!canChangeContent(user.role)) {
        return errorResponse("You do not have permission to finish crafting immediately");
      }
      if (!craftingItem) {
  ...
  finishImbuingImmediately: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [user, userItems] = await Promise.all([
        // Get user data
        fetchUser(ctx.drizzle, ctx.userId),
        // Get all user items with imbuements
        fetchUserItems(ctx.drizzle, ctx.userId),
      ]);
      ...
      // Guards
      if (!canChangeContent(user.role)) {
        return errorResponse("You do not have permission to finish imbuing immediately");
      }
