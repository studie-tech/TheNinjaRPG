...
import { calcMaxItems, calcMaxEventItems, calcMaxMaterials, calcMaxHouseMaterials } from "@/libs/item";
  ...
  toggleStoreItem: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (userItemResult.storedAtHome) {
        ...
        if (userItemResult.item.itemType === "MATERIAL" && nMaterials >= calcMaxMaterials(user)) {
        ...
        if (userItemResult.item.itemType === "MATERIAL") {
          ...
          const storedMaterials = storedItems.filter((ui) => ui.item.itemType === "MATERIAL").length || 0;
          if (storedMaterials >= calcMaxHouseMaterials(user, HomeTypeDetails[user.homeType].storage)) {
