                    ...
                    availableEvolutions.map((evo) => {
                      const effectiveLevel =
                        userjutsu.finishTraining && userjutsu.finishTraining > now
                          ? userjutsu.level - 1
                          : userjutsu.level;
                      const meetsEvoSkill = meetsRequiredSkill(
                        evo.requiredSkillId,
                        activatedSkillIds,
                      );
...
const categorizeJutsus = (
  ...
  for (const uj of userJutsus) {
    // Equipped jutsu stay in the equipped section even when skill-restricted;
    // they still occupy a loadout slot (curEquip) and show a skill warning.
    if (uj.equipped) {
