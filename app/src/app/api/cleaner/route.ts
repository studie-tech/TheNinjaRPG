  ...
  try {
    ...
    await drizzleDB.execute(
      sql`UPDATE ${userData} u SET u.federalStatus = 'NONE' WHERE u.federalStatus != 'NONE' AND NOT EXISTS (
        ...
        SELECT 1 FROM ${storePurchase} s WHERE s.userId = u.userId AND s.federalStatus IS NOT NULL AND s.isSandbox = 0 AND s.revokedAt IS NULL AND s.createdAt >= CURRENT_TIMESTAMP(3) - INTERVAL 63 DAY
      )`,
    );
